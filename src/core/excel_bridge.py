...
def normalize_str(s):
    if not s:
        return ""
    # Convert to string and decode accents
    s = str(s)
    s = unicodedata.normalize('NFD', s).encode('ascii', 'ignore').decode('utf-8')
    s = s.lower()
    # Remove non-alphanumeric chars (parentheses, commas, dots, dashes) but keep spaces and slashes
    s = re.sub(r'[^a-z0-9 /]', '', s)
    # Collapse spaces
...
def _log_audit(msg):
    try:
        ...
        log_path = os.path.join(log_dir, "excel_bridge.log")
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} {msg}\n")
    except Exception as e:
        print(f"Error writing to excel_bridge.log: {e}")

def export_snapshot_to_master(snapshot_id):
    _log_audit(f"START: Exporting snapshot {snapshot_id} to Excel Master.")
    ...
    try:
        ...
        if not snap:
            _log_audit(f"ERROR: Snapshot ID {snapshot_id} not found in DB.")
            session.close()
            return False
        ...
        try:
            ...
            _log_audit(f"ERROR: Could not open Excel file at {file_path}. Details: {e}")
            session.close()
            return False
        ...
        for c in range(1, 100):
            ...
            if val is not None:
                if isinstance(val, str) and normalize_str(val) == normalize_str(tar_month):
                    ...
                    # Datetime object mapping: e.g. May/20
                    # Convert to string or check year/month matches
                    val_str = val.strftime("%Y-%m")
                    # Check if year-month matches target month
                    if val_str == snap.month_id:
                        col_idx = c
                        break
        ...
        if col_idx == -1:
            ...
            for c in range(2, 100):
                if sheet.cell(row=1, column=c).value is None:
                    ...
                    break
            if col_idx == -1:
                col_idx = 100  # Fallback
            
            sheet.cell(row=1, column=col_idx).value = tar_month
            _log_audit(f"COLUMN_CREATE: Created new column {col_idx} for month '{tar_month}'")
        else:
            _log_audit(f"COLUMN_MATCH: Found existing column {col_idx} for month '{tar_month}'")
        ...
        for b in bals:
            ...
            if 'ibkr' in norm_name and 'rafa' in norm_name:
                ...
                val_map['aportado ibkr cris'] = v_ap
                val_map['profit ibkr cris'] = v_pr
            elif 'monetario' in norm_name and 'cris' in norm_name:
                val_map['fondo monetario cris'] = v_cur
            else:
                # Bank accounts or debts
                if 'formacion' in norm_name:
                    val_map['cuenta formacion rafa'] = v_cur
                elif 'inversion' in norm_name and 'rafa' in norm_name:
                    val_map['cuenta inversion rafa'] = v_cur
                elif 'insta' in norm_name:
                    val_map['cuenta ahorro (instan saving) cris'] = v_cur
                elif 'monthly' in norm_name:
                    val_map['cuenta ahorro (monthly saving) cris'] = v_cur
                elif 'caixa' in norm_name:
                    val_map['cuenta la caixa cris (eur)'] = v_cur
                elif 'bankia' in norm_name:
                    val_map['cuenta bankia rafa (eur)'] = v_cur
                elif 'hamburgh' in norm_name and acc.type == 'real_estate':
                    val_map['casa hamburgh place'] = v_cur
                elif 'hamburgh' in norm_name and acc.type == 'mortgage':
                    val_map['hipoteca hamburgh place'] = v_cur
                elif 'deuda equity' in norm_name or 'interna rafa' in norm_name:
                    val_map['deuda interna rafa'] = v_cur
                else:
                    val_map[acc.name] = v_cur
        ...
        for row in range(1, 150):
            ...
            for k, val in val_map.items():
                nk = normalize_str(k)
                # Exact or tolerant partial match logic
                if nk == nc or (nk in nc and len(nk) > 5) or (nc in nk and len(nc) > 5):
                    cell = sheet.cell(row=row, column=col_idx)
                    cell.value = val
                    if isinstance(val, (int, float)):
                        cell.number_format = '#,##0'
                    matched_keys.add(k)
                    _log_audit(f"WRITE: Row {row:03d} | Excel cell: '{cell_val}' -> Matched with: '{k}' | Value: {val}")
                    break
                    
        # Log unmatched items for debugging
        unmatched_keys = set(val_map.keys()) - matched_keys
        for uk in unmatched_keys:
            _log_audit(f"WARNING: Key '{uk}' (Value: {val_map[uk]}) did not match any row in Excel column 1.")
                    
        try:
            wb.save(file_path)
            _log_audit(f"SUCCESS: Saved Excel file successfully at {file_path}.")
            session.close()
            return True
        except Exception as e:
            _log_audit(f"ERROR: Failed to save Excel file (Is it open by Excel?). Details: {e}")
            session.close()
            return False
            
    except Exception as e:
        _log_audit(f"CRITICAL: Unexpected error in excel_bridge: {e}")
        session.close()
        return False
